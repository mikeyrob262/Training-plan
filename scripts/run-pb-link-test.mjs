...
const st = {
  rides: [
    { stravaId: 900, date: '2026-03-01', distance: 6.2, sportType: 'Run' },   // position 0 ON PURPOSE
    { stravaId: 901, date: '2023-05-05', distance: 6.2, sportType: 'Run' },
    { stravaId: 902, date: '2026-07-07', distance: 4.0, sportType: 'Run', deleted: true }
  ]
};
...
const M = new Function(
  ...
  'openRideDetail', 'openDesktopRideDetail', 'document', 'window', '_runAll_', '_runCard_', 'getTodayKey',
  asServed(
    ...
    'return { _runRefFor_, _runOpenRef_, _prCompute_, _prSection_, _run10kPlan_, _run10kCardHTML_, rideRefAttr_, rideRefOk_ };'
  ))(
  ...
  (t, s, b) => '<card t="' + t + '">' + b + '</card>', () => '2026-08-13');
...
console.log('\n' + Y + '=== a stat resolves back to the run that set it ===' + X);
{
  ...
  ok('a projection with no stravaId at all is refused, not guessed',
...
console.log('\n' + Y + '=== no dead clicks: the 2015 career best is not in the library ===' + X);
{
  const board = M._prCompute_();
  const row = board.rows.filter((r) => r.ev.id === '10k')[0];
  ok('the board still SHOWS the career best', !!row && row.career.date.slice(0, 4) === '2015');
  ok('...but it carries no reference', !M.rideRefOk_(row.career.ref));
  ok('...and the band best does carry one', M.rideRefOk_(row.band.ref));
  const html = M._prSection_();
  ok('the unresolvable career best renders as plain text', !/onclick[^>]*>4?1?:?12/.test(html));
  ok('the resolvable rows still render as links', /_runOpenRef_\(/.test(html));
  // Only the three PICKED entries per event are resolved - resolving every candidate would be a
  // linear st.rides scan per run, thousands per render, to keep at most three answers.
  ok('candidates that were not picked were never resolved',
     board.rows.every((r) => r.career.ref !== undefined && r.band.ref !== undefined));
}
